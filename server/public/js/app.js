...
const SPOTS = [
  ...
  { day: 2, city: '방콕', name: '왓포 (와불사원)', note: '왕궁 도보 5분. 15m 황금 와불상과 전통 마사지 스쿨로 유명.', map: 'Wat Pho Bangkok', tint: 'bl-300' },
  { day: 2, city: '방콕', name: '카오산 로드', note: '배낭여행자 거리의 상징. 프리마켓과 길거리 음식, 야시장 분위기.', map: 'Khao San Road Bangkok', tint: 'sand' },
  { day: 3, city: '방콕', name: '조드페어스 야시장', note: '목요일부터 일요일까지 열리는 라마9의 야시장.', map: 'Jodd Fairs Rama 9 Bangkok', tint: 'sand' },
  { day: 4, city: '파타야', name: '파타야 수상시장', note: '수상가옥 사이를 오가며 즐기는 플로팅마켓.', map: 'Pattaya Floating Market', tint: 'bl-200' },
  { day: 4, city: '파타야', name: '파타야 나이트 바자', note: '해변가 야시장. 스트리트푸드와 기념품 쇼핑.', map: 'Pattaya Night Bazaar', tint: 'bl-100' },
...
const MEALS = [
  { day: 2, city: '방콕', name: '쿤댕 꾸어이짭 유안', note: '카오산 인근. 베트남식 롤 쌀국수 맛집.', map: 'Khun Daeng Kuay Jub Yuan Bangkok', tint: 'bl-100' },
  { day: 2, city: '방콕', name: '엠쿼티어 헬릭스 푸드코트', note: '다양한 로컬 음식을 한자리에서. 1인 150~300바트.', map: 'EmQuartier Helix Quartier Food Court Bangkok', tint: 'sand' },
  { day: 4, city: '파타야', name: '터미널21 파타야 푸드코트', note: '팟타이 50바트, 카오카무 60바트 등 가성비 좋은 로컬 음식.', map: 'Terminal 21 Pattaya', tint: 'bl-200' },
  { day: 5, city: '파타야', name: '뿌펜 파타야', note: '게커리로 유명한 시푸드 맛집. 1인 약 500~1,000바트.', map: 'Pupen Seafood Pattaya', tint: 'bl-300' },
  { day: 8, city: '방콕', name: '팁싸마이 팟타이', note: '아이콘시암 인근, 방콕을 대표하는 팟타이 노포.', map: 'Thipsamai Pad Thai Bangkok', tint: 'bl-100' },
  { day: 8, city: '방콕', name: '야오와랏 차이나타운 야시장', note: '거리 전체가 야시장. 다양한 길거리 음식이 모여있음.', map: 'Yaowarat Road Chinatown Bangkok', tint: 'bl-200' },
];
...
function renderMeals() {
  ...
  if (!grid) return;
  grid.innerHTML = MEALS.map((p) => `
    <a class="spot-card" data-tint="${p.tint}" href="${mapLink(p.map)}" target="_blank" rel="noopener">
      <span class="spot-icon"><svg><use href="#i-meal"></use></svg></span>
      <span class="spot-eyebrow">Day ${p.day} · ${esc(p.city)}</span>
      <span class="spot-name">${esc(p.name)}</span>
      <span class="spot-note">${esc(p.note)}</span>
    </a>`).join('');
